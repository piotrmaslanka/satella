...
class Proxy(tp.Generic[T]):
    ...
    def _get_obj(self) -> object:
        """Return a self.__obj to use instead"""
        return self.__obj
    ...
    def __delitem__(self, key) -> None:
        del self._get_obj()[key]

    def __setattr__(self, key, value) -> None:
        if key in _SETTABLE_KEYS:
            super().__setattr__(key, value)
        else:
            setattr(self._get_obj(), key, value)

    def __getattr__(self, item):
        return getattr(self._get_obj(), item)

    def __delattr__(self, item) -> None:
        delattr(self._get_obj(), item)

    def __int__(self) -> int:
        return int(self._get_obj())

    @rethrow_as(AttributeError, TypeError)
    def __float__(self) -> float:
        return float(self._get_obj())

    @rethrow_as(AttributeError, TypeError)
    def __complex__(self) -> complex:
        return complex(self._get_obj())
    ...
    def __divmod__(self, other) -> tp.Tuple[Number, Number]:
        return divmod(self._get_obj(), other)
    ...
    def __rdivmod__(self, other):
        return divmod(other, self._get_obj())
    ...
    def __enter__(self):
        return self._get_obj().__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        return self._get_obj().__exit__(exc_type, exc_val, exc_tb)

    def __repr__(self) -> str:
        return repr(self._get_obj())

    def __oct__(self) -> str:
        return oct(self._get_obj())

    def __hex__(self) -> str:
        return hex(self._get_obj())

    @wrap_operation
    def __abs__(self):
        return abs(self._get_obj())

    def __bool__(self) -> bool:
        return bool(self._get_obj())

    def __format__(self, format_spec):
        return self._get_obj().__format__(format_spec)

    def __next__(self):
        return next(self._get_obj())
    ...
    def __await__(self):
        return self._get_obj().__await()

    def __aenter__(self):
        return self._get_obj().__aenter__()

    def __aexit__(self, exc_type, exc_val, exc_tb):
        return self._get_obj().__aexit__(exc_type, exc_val, exc_tb)

    def __aiter__(self):
        return self._get_obj().__aiter__()

    def __reversed__(self):
        return reversed(self._get_obj())

    @wrap_operation
    def __pow__(self, power, modulo=None):
        return pow(self._get_obj(), power, modulo)
    ...
    def __index__(self) -> int:
        return self._get_obj().__index__()

    @wrap_operation
    @rethrow_as(AttributeError, TypeError)
    def __round__(self, n: int = 0):
        return round(self._get_obj(), n)

    @wrap_operation
    @rethrow_as(AttributeError, TypeError)
    def __trunc__(self):
        if hasattr(self._get_obj(), '__trunc__'):
            result = self._get_obj().__trunc__()
        else:
            result = math.trunc(self._get_obj())
        return result

    @wrap_operation
    @rethrow_as(AttributeError, TypeError)
    def __floor__(self):
        if hasattr(self._get_obj(), '__floor__'):
            result = self._get_obj().__floor__()
        else:
            result = math.floor(self._get_obj())
    ...
    @rethrow_as(AttributeError, TypeError)
    def __ceil__(self):
        if hasattr(self._get_obj(), '__ceil__'):
            result = self._get_obj().__ceil__()
        else:
            result = math.ceil(self._get_obj())
        return result
    ...
    def __concat__(self, other):
        return self._get_obj().__concat__(other)

    @rethrow_as(AttributeError, TypeError)
    def __iconcat__(self, other) -> 'Proxy':
        self._get_obj().__iconcat__(other)
        return self

    @rethrow_as(AttributeError, TypeError)
    def __length_hint__(self):
        return self._get_obj().__length_hint__()
